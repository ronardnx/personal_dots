...
complete -c tar -l level -d 'Set dump level for a created listed-incremental archive'
...
complete -c tar -l sparse-version -d 'Set which version of the sparse format to use'
...
complete -c tar -s p -l preserve-permissions -l same-permissions -d 'Set permissions of extracted files to those recorded in the archive (default …'
complete -c tar -l same-owner -d 'Try extracting files with the same ownership as exists in the archive (defaul…'
complete -c tar -s s -l preserve-order -l same-order -d 'Tell tar that the list of file names to process is sorted in the same order a…'
...
complete -c tar -l xattrs-include -d 'Specify the include pattern for xattr keys.   PATTERN is a globbing pattern'
...
complete -c tar -s A -l catenate -l concatenate -d 'Append archives to the end of another archive'
...
complete -c tar -s '?' -l help -d 'Display a short option summary and exit'
complete -c tar -l usage -d 'Display a list of available options and exit'
